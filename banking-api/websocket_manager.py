...
from api.domains.transaction.models import Transaction
import logging
    ...
    async def send_personal_message(self, message: str, websocket: WebSocket):
        ...
        try:
            ...
            if "WebSocket is not connected" in str(e) or "close message has been sent" in str(e):
                ...
                logger.error(f"Runtime error sending message to WebSocket: {e}")
                self.disconnect(websocket)
        except Exception as e:
            logger.error(f"Unexpected error sending message to WebSocket: {e}")
            self.disconnect(websocket)
    ...
    async def _send_to_connections(self, connections: Set[WebSocket], message: str):
        ...
        for connection in connections.copy():
            try:
                ...
                if "WebSocket is not connected" in str(e) or "close message has been sent" in str(e):
                    logger.debug(f"WebSocket already closed during broadcast: {e}")
                    disconnected.add(connection)
                else:
                    logger.error(f"Runtime error during broadcast: {e}")
                    disconnected.add(connection)
            except Exception as e:
                logger.error(f"Unexpected error sending message to WebSocket: {e}")
                disconnected.add(connection)
